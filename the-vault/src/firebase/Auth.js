...
function useProvideAuth(){
   ...
   const signUp = ( auth , email, password , displayName) => createUserWithEmailAndPassword(email , password).then(({user})=>{
   ...
   const signIn = (auth , email ,password ) => signInWithEmailAndPassword(email , password).then(({user}) =>  {
    setUser(user)
    return user;
   })
